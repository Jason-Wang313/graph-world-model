...
\newcommand{\bestn}{Best-of-$N$}
\newcommand{\utility}{U}
\newcommand{\score}{S}
\newcommand{\selected}{\mathrm{sel}}

\title{When Graph Worlds Overfit Their Own Physics}
...
\begin{abstract}
\bestn{} inference is tail selection under a model score: sample $N$ candidate futures, score each one, and execute the highest-scoring candidate.  If the score omits constraints that matter under execution, larger $N$ can concentrate selection on candidates that are good only inside the model's imagined physics.  We study this failure mode in a CPU-local synthetic graph-physics suite where an imagined mass-spring graph scores rollouts and a separate true graph evaluates utility.  We prove an exact finite-pool, tie-aware law for the expected utility selected by \bestn{}, validate it empirically with mean absolute error $0.001341$, and audit graph-energy, action, calibration-gap, constraint-probe, combined-repair, adaptive-gating, and learned-lite calibration controls.  At $N=64$, adaptive gating improves selected real utility over raw selection by $0.00701$ on 320 paired cases and by $0.01447$ on 155 hard cases, with zero negative deltas in this audit.  The evidence is deliberately narrow: synthetic graph physics only, no real-robot validation, no external simulator benchmark, and no claim that larger $N$ universally improves deployed behavior.
\end{abstract}

\section{Introduction}

Increasing $N$ in a \bestn{} controller is often described as ``more search.''  A more precise description is tail selection under a score.  The controller samples $N$ futures, evaluates each with an internal score, and chooses the maximum.  If the high-score tail is also high-utility, increasing $N$ can help.  If the score is misspecified in the tail, increasing $N$ can make the selected future more extreme along precisely the dimensions the score fails to penalize.
...
The paper contributes: (1) an exact finite-pool law for tie-aware \bestn{} selection; (2) a graph-physics stress suite over five graph families, five hidden failures, and three stress levels; (3) selector, repair, adaptive-gate, and learned-lite calibration audits tied to CSV artifacts; and (4) an explicit limitation and reproducibility record.

\section{Related Work}

\bestn{} and reranking are common inference-time tools.  Reward-model or verifier-based selection appears in summarization, mathematical reasoning, web-assisted question answering, and instruction following \citep{stiennon2020learning,cobbe2021training,nakano2021webgpt,ouyang2022training}.  Our focus is not language generation, but the same order-statistic pressure: the selected sample is an extreme under a learned or specified score \citep{david2003order}.
...
Let a finite pool contain $m$ candidates.  Candidate $i$ has model score $\score_i$ and evaluator utility $\utility_i$.  \bestn{} draws $N$ candidates independently with replacement from the pool and selects a sampled candidate with maximal score.  If several sampled positions tie for maximal score, the selector breaks ties uniformly among tied sampled positions.

\begin{theorem}[finite tie-aware \bestn{} law]
\label{thm:law}
...
The theorem is independent of graph physics.  It says that \bestn{} shifts probability mass toward score-tie groups in the score tail.  If those groups have lower mean evaluator utility, larger $N$ can hurt utility or leave an oracle gap even as selected score rises.  In the artifact, the theorem is implemented in \texttt{src/graph\_physics\_best\_of\_n/theory.py}, tested in \texttt{tests/test\_theory.py}, and validated in \texttt{results/tables/exact\_law\_validation.csv}.
...
\end{center}
\caption{selected-tail failure in the synthetic suite.  Raw \bestn{} selection raises the internal score tail while true utility remains below the oracle tail in controlled graph-physics conditions.}
\label{fig:tail}
\end{figure}
...
The selector coefficients are fixed in \texttt{src/graph\_physics\_best\_of\_n/selection.py}.  The hidden-risk proxy is
\[
R_{\mathrm{hidden}} =
